		...
		$('#b2').click(function() {
			$.ajax({
				...
				success: function(json) {
					//alert(json)
					//$('#result').text(json)
					console.log(json) //'json을 프린트할 일이 있으면 브라우저 콘솔에 찍는것이 편하다.'
		...
		$('#b3').click(function() {
			$.ajax({
				...
				success: function(array) {
					//alert(json2)
					//$('#result').text(json2)
					$(array).each(function(i, json) { // function(인덱스저장변수,하나씩꺼내서넣어줄변수) 순서대로 써주어야함.
						id_value = json.id
						title_value = json.title
						content_value = json.content
						writer_value = json.writer		
...
<body>
	...
	<hr color="red">
	<div id="result" style="background: yellow; width=800px; hegith=800px;">JSON결과 들어가는 곳.<hr></div>
</body>
</html>